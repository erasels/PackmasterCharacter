...
public class DeadOrAliveActionAction extends AbstractGameAction {
    ...
    public void update() {
        if (this.duration == 0.1F && this.target != null && !isDeadOrEscaped(this.target)) {
            AbstractDungeon.effectList.add(new FlashAtkImgEffect(this.target.hb.cX + MathUtils.random(-8f, 8f), this.target.hb.cY + MathUtils.random(-8f, 8f), AbstractGameAction.AttackEffect.BLUNT_LIGHT));
