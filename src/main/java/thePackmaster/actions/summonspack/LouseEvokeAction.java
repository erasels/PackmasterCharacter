    ...
    public void update() {
        ...
        if (duration <= DURATION - thunkTiming && !thunkEffect) {
            ...
            if (target != null && target.currentHealth > 0 && adp() != null) {
                DamageInfo info = new DamageInfo(adp(), louse.evokeAmount, DamageInfo.DamageType.THORNS);
                AbstractGameAction action = new DamageAction(target, info, AttackEffect.BLUNT_HEAVY, true);
                ColoredDamagePatch.DamageActionColorField.rainbow.set(action, true);
                att(action);
            }
