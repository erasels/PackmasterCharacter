...
public class SurgePower extends AbstractPackmasterPower {
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if (!card.purgeOnUse && card.isInnate && this.amount > 0) {
            ...
            this.amount--;
            if (this.amount == 0)
                addToTop((AbstractGameAction)new RemoveSpecificPowerAction(this.owner, this.owner, makeID("SurgePower")));
        }
        updateDescription();
    }
    ...
    public void atEndOfTurn(boolean isPlayer) {
        if (isPlayer)
            addToBot((AbstractGameAction)new RemoveSpecificPowerAction(this.owner, this.owner, makeID("SurgePower")));
    }
}
